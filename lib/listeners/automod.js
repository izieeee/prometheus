...
const blacklist = new Set(
  (process.env.AUTOMOD_BLACKLIST || '').split(',').map(s => s.trim()).filter(Boolean)
);
...
export default async function automodListener({ event, logger }) {
  ...
  if (blacklist.has(event.channel)) return;
    ...
    if (fuz(event.text, word)) {
      await Promise.all([
        bot.chat.postMessage({
          channel: event.user,
          text: `You have used a prohibited term. This infraction has been logged. Please adhere to the community guidelines.`,
        }).catch(e => {
