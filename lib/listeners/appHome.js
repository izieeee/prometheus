...
function banBlocks(bans) {
  const blocks = [hdr('Channel Bans')];
  if (!bans.length) {
    blocks.push(md('_No active bans. Peace reigns... for now._'));
  } else {
    const byChannel = {};
    for (const b of bans) {
    ...
    for (const [ch, entries] of Object.entries(byChannel)) {
      ...
      for (const b of entries) {
        ...
        blocks.push({
          type: 'section',
          text: { type: 'mrkdwn', text: `:bonk: <@${b.user_id}> banned by <@${b.banned_by}> (${expiry})${reason}` },
          accessory: {
            type: 'button',
            text: { type: 'plain_text', text: 'Unban' },
            style: 'danger',
            action_id: `home_unban_${b.user_id}_${b.channel_id}`,
            confirm: {
              title: { type: 'plain_text', text: 'Unban User?' },
              text: { type: 'plain_text', text: `Remove the ban on this user in this channel?` },
              confirm: { type: 'plain_text', text: 'Unban' },
              deny: { type: 'plain_text', text: 'Keep banned' },
            },
          },
        });
      }
