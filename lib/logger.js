...
export async function logBan(client, { channel, user, bannedBy, reason, expires, unbanned }) {
  ...
  const action = unbanned
    ? `:okay-1: <@${bannedBy}> unbanned <@${user}> from <#${channel}>`
    : `:bonk: <@${bannedBy}> banned <@${user}> from <#${channel}>${expires ? ` until <!date^${expires}^{date_short_pretty} at {time}|${new Date(expires * 1000).toUTCString()}>` : ''}`;
  ...
  await getLogClient(client).chat.postMessage({
    channel: LOG_CHANNEL,
    text: unbanned ? `User unbanned in channel` : `User banned in channel`,
    blocks,
  });
}
